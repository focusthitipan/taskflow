...
<p align="center">
  ...
  <img src="screens/kanban-v2.jpg" alt="Dashboard Kanban Board" width="100%" />
</p>

<p align="center">
  <strong>📅 My Tasks — มุมมองปฏิทิน</strong><br/>
  <img src="screens/calendar-v2.jpg" alt="My Tasks Calendar View" width="100%" />
</p>

<p align="center">
  <strong>👥 Team — สมาชิกทีม & ภาระงาน</strong><br/>
  <img src="screens/team-v2.jpg" alt="Team Members & Workload" width="100%" />
</p>

<p align="center">
  <strong>📊 Daily Graph — Productivity · Energy · Focus</strong><br/>
  <img src="screens/daily-graph-v2.jpg" alt="Daily Graph 3-axis Chart" width="100%" />
</p>

<p align="center">
  <strong>👑 User Management — จัดการผู้ใช้ (Admin)</strong><br/>
  <img src="screens/user-management-v2.jpg" alt="User Management Admin" width="100%" />
</p>

<p align="center">
  <strong>🔐 หน้าเข้าสู่ระบบ</strong><br/>
  <img src="screens/login-v2.jpg" alt="Login Page" width="100%" />
</p>
...
## 📊 วิธีการคำนวณกราฟรายวัน (Daily Graph)

กราฟรายวันที่ `/graph` แสดงข้อมูลประสิทธิภาพ 3 แกนตลอด 24 ชั่วโมงของวันที่เลือก โดยประมวลผลจากข้อมูลจริงในฐานข้อมูลแบบ **real-time** (ไม่มีการ cache)

### 🔍 แหล่งข้อมูลที่ใช้

API `/api/graph/daily` จะดึงข้อมูล 4 ประเภทจากฐานข้อมูลสำหรับวันที่ที่เลือก:

| ข้อมูล | ตาราง | เงื่อนไข | นำไปใช้กับ |
|---|---|---|---|
| **Activity Logs** | `ActivityLog` | `createdAt` อยู่ในวันที่เลือก | Energy |
| **Comments** | `Comment` | `createdAt` อยู่ในวันที่เลือก | Energy |
| **Done Tasks** | `Task` | `status = "done"` และ `updatedAt` อยู่ในวันที่เลือก | Productivity |
| **Active Tasks** | `Task` | `progress > 0` และ `updatedAt` อยู่ในวันที่เลือก | Focus |

### 📐 กระบวนการคำนวณ

ข้อมูลที่ดึงมาจะถูกนับแยกตามชั่วโมง (0–23 ตาม UTC) จากนั้นนำไปคำนวณ 3 ค่าดังนี้:

---

### 1. 📈 Productivity (0–100%)

วัดจาก **จำนวนงานที่เสร็จ (done tasks)** ในวันนั้น คูณกับเส้นโค้งประสิทธิภาพตามช่วงเวลา (circadian rhythm curve):

```
baseProductivity = (doneCount / 10) × 15 × productivityCurve(hour)
```

**เส้นโค้งประสิทธิภาพตามชั่วโมง** (productivityCurve) จำลองช่วงเวลาที่คนทำงานได้ดี:
| ช่วงเวลา | สูตร | ลักษณะ |
|---|---|---|
| ก่อน 9:00 | `0` | นอกเวลาทำงาน |
| 09:00–11:00 | `0.4 + t×0.3` | เพิ่มขึ้นสูงสุดที่ 11:00 (peak = 1.0) |
| 11:00–13:00 | `1.0 - t×0.35` | ลดลงช่วงกลางวัน (post-lunch dip) |
| 13:00–16:00 | `0.3 + t×0.23` | ฟื้นตัวช่วงบ่าย สูงสุดที่ 16:00 |
| 16:00–18:00 | `1.0 - t×0.5` | ลดลงช่วงเย็น |
| หลัง 18:00 | `0` | นอกเวลาทำงาน |

**การ Normalize:** หลังจากคำนวณค่าดิบทั้ง 24 ชั่วโมงแล้ว หากค่า max < 100 จะถูก **scale ขึ้น** ให้ max = 100% เพื่อให้กราฟใช้พื้นที่แนวตั้งเต็มช่วง:

```
normalizedProductivity[i] = round((raw[i] / maxRaw) × 100)
```

---

### 2. ⚡ Energy (−100 ถึง +100)

วัดจาก **จำนวนกิจกรรม (activity logs + comments)** ในแต่ละชั่วโมง เทียบกับค่าเฉลี่ยของวัน:

```
baseEnergy = round((hourlyActivity / avgActivityPerBin) × 40 − 20)
```
โดยที่ `avgActivityPerBin = ceil(totalActivity / 10)`

หมายความว่า:
- ถ้าชั่วโมงนั้นมีกิจกรรม = ค่าเฉลี่ย → `40 − 20 = 0` (ปกติ)
- ถ้าชั่วโมงนั้นมีกิจกรรม = 0 → `0 − 20 = −20` (ต่ำ)
- ถ้าชั่วโมงนั้นมีกิจกรรม = 2× ค่าเฉลี่ย → `80 − 20 = 60` (สูง)

**ตัวปรับตามช่วงเวลา (Time-of-day modifiers):**
| ช่วงเวลา | ตัวปรับ | คำอธิบาย |
|---|---|---|
| 00:00–06:00 | `−40` | ช่วงหลับ — กิจกรรมน้อยเป็นเรื่องปกติ |
| 07:00–08:00 | `+10` | เช้าตรู่ — เริ่มต้นวัน |
| 09:00–11:00 | `+20` | ช่วงเช้า — พลังงานสูงสุด |
| 12:00–17:00 | `+10` | บ่าย — พลังงานปานกลาง |
| 18:00–20:00 | `−5` | เย็น — เริ่มลด |
| 21:00–23:00 | `−35` | กลางคืน — ควรพักผ่อน |

> ⚠️ Energy **ไม่**ผ่านการ normalize — ค่าที่ได้คือค่าจริงตามสเกล −100 ถึง +100

---

### 3. 🎯 Focus (0–10)

วัดจาก **จำนวนงานที่กำลังดำเนินการ (active tasks, progress > 0)** คูณกับเส้นโค้งสมาธิตามช่วงเวลา:

```
baseFocus = (activeCount / 10) × 2.5 × focusCurve(hour)
```

**เส้นโค้งสมาธิ (focusCurve)** — โฟกัสจะ peak คนละช่วงกับ productivity:
| ช่วงเวลา | สูตร | ลักษณะ |
|---|---|---|
| ก่อน 8:00 | `0` | นอกเวลาทำงาน |
| 08:00–11:00 | `0.3 + t×0.23` | เพิ่มขึ้น สูงสุดที่ 11:00 |
| 11:00–13:00 | `1.0 − t×0.25` | ลดลงเล็กน้อยช่วงกลางวัน |
| 13:00–18:00 | `0.5 + t×0.17` | ฟื้นตัวและคงที่ช่วงบ่าย |
| หลัง 18:00 | `0` | นอกเวลาทำงาน |

**การ Normalize:** เช่นเดียวกับ Productivity — scale ให้ max = 10:

```
normalizedFocus[i] = round((raw[i] / maxRaw) × 10 × 10) / 10
```
...
| เมตริก | สเกล | อิงจาก | Normalize? |
|---|---|---|---|
| **Productivity** | 0–100% | Done Tasks × productivity curve | ✅ Scale ให้ max = 100 |
| **Energy** | −100 ถึง +100 | Activity count ต่อชั่วโมง + time modifiers | ❌ |
| **Focus** | 0–10 | Active Tasks × focus curve | ✅ Scale ให้ max = 10 |

ทั้ง 3 ค่าถูกคำนวณสำหรับทุกชั่วโมง (00:00–23:00) และแสดงผลในกราฟเส้น 3 แกน (Triple Y-Axis Composed Chart) ด้วย Recharts
